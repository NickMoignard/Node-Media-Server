...
//
var Bitop = require('./node_core_bitop');
var AAC_SAMPLE_RATE = [
...
var AUDIO_SOUND_RATE = [
    5512, 11025, 22050, 44100
];
var VIDEO_CODEC_NAME = [
    '',
    'Jpeg',
    'Sorenson-H263',
    'ScreenVideo',
...
function getObjectType(bitop) {
    var audioObjectType = bitop.read(5);
...
function readAACSpecificConfig(aacSequenceHeader) {
    var info = {};
    var bitop = new Bitop(aacSequenceHeader);
    bitop.read(16);
...
function readH264SpecificConfig(avcSequenceHeader) {
    var info = {};
    var profile_idc, width, height, crop_left, crop_right, crop_top, crop_bottom, frame_mbs_only, n, cf_idc, num_ref_frames;
...
function HEVCParseSPS(SPS, hevc) {
    ...
    for (var i = 2; i < NumBytesInNALunit; i++) {
        if (i + 2 < NumBytesInNALunit && bitop.look(24) == 0x000003) {
            ...
            rbsp_array.push(bitop.read(8));
            i += 2;
            var emulation_prevention_three_byte = bitop.read(8); /* equal to 0x03 */
        }
        else {
            rbsp_array.push(bitop.read(8));
        }
    }
    var rbsp = Buffer.from(rbsp_array);
    var rbspBitop = new Bitop(rbsp);
...
function readHEVCSpecificConfig(hevcSequenceHeader) {
    ...
    do {
        ...
        var numOfArrays = hevcSequenceHeader[22];
        var p = hevcSequenceHeader.slice(23);
...
function readAVCSpecificConfig(avcSequenceHeader) {
    var codec_id = avcSequenceHeader[0] & 0x0f;
    if (codec_id == 7) {
...
module.exports = {
    AUDIO_SOUND_RATE: AUDIO_SOUND_RATE,
    AUDIO_CODEC_NAME: AUDIO_CODEC_NAME,
    VIDEO_CODEC_NAME: VIDEO_CODEC_NAME,
    readAACSpecificConfig: readAACSpecificConfig,
    getAACProfileName: getAACProfileName,
    readAVCSpecificConfig: readAVCSpecificConfig,
    getAVCProfileName: getAVCProfileName,
};
