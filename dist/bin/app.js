...
nms.run();
nms.on('preConnect', function (id, args) {
    console.log('[NodeEvent on preConnect]', "id=" + id + " args=" + JSON.stringify(args));
    // let session = nms.getSession(id);
    // session.reject();
});
nms.on('postConnect', function (id, args) {
    console.log('[NodeEvent on postConnect]', "id=" + id + " args=" + JSON.stringify(args));
});
nms.on('doneConnect', function (id, args) {
...
nms.on('prePlay', function (id, StreamPath, args) {
    console.log('[NodeEvent on prePlay]', "id=" + id + " StreamPath=" + StreamPath + " args=" + JSON.stringify(args));
    // let session = nms.getSession(id);
    // session.reject();
});
nms.on('postPlay', function (id, StreamPath, args) {
    console.log('[NodeEvent on postPlay]', "id=" + id + " StreamPath=" + StreamPath + " args=" + JSON.stringify(args));
});
nms.on('donePlay', function (id, StreamPath, args) {
