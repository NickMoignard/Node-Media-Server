...
export class rtmpHevcConfig extends ffmpegConfigInterface {
  constructor(
    ...
    this.hls_flags = config?.hls_flags || 'append_list+independent_segments+program_date_time+second_level_segment_index'
  ...
  args() {
    ...
    const outpath = `../../media${this.streamPath}/${this.scale.h}p.m3u8`
    const remaining_params = [
      ...
      '-c:v', this.vc,
      '-preset', this.video_codec_preset,
      '-tune', this.tune,
...
export class WSHevcConfig extends ffmpegConfigInterface {
  ...
  args() {
    ...
    const remaining_params = [
      ...
      '-hwaccel_output_format', this.hwaccel_output_format, // keeps decoded frames in GPU memory
      // input
      '-i', this.source, //"E:\\Vids\\Vlogs\\october\\18th\\GOPR9685.MP4",
      // '-resize', `${this.scale.w}x${this.scale.h}`,
      '-c:a', this.audio_codec,
      '-c:v', this.vc,
      '-preset', this.video_codec_preset,
